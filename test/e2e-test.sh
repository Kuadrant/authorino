...
kubectl port-forward --namespace authorino deployment/envoy 8081:8000 &
...
[ $(curl -o /dev/null -L -s -w "%{http_code}" -H 'Host: echo-api' -H "Authorization: Bearer $ACCESS_TOKEN_JOHN" http://localhost:8081/pets) -eq 200 ];
[ $(curl -o /dev/null -L -s -w "%{http_code}" -H 'Host: echo-api' -H "Authorization: Bearer $ACCESS_TOKEN_JOHN" http://localhost:8081/pets/1) -eq 200 ];
[ $(curl -o /dev/null -L -s -w "%{http_code}" -H 'Host: echo-api' -H "Authorization: Bearer $ACCESS_TOKEN_JOHN" http://localhost:8081/stats) -eq 401 ];
