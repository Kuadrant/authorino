...
# Run with specific envtest binaries (e.g., for Kubernetes 1.35.0)
KUBEBUILDER_ASSETS=$(bin/setup-envtest use -p path 1.35.0) go test ./tests/cel/... -v
...
K8S_VERSION=1.35.0 make test-cel
```
...
- **`K8S_VERSION`**: Kubernetes version for envtest (e.g., `1.35.0`). Defaults to the repository-pinned `1.35.0` (set in the Makefile) when run via `make`; `setup-envtest` invoked directly falls back to the latest GA version
...
# Set the path
export KUBEBUILDER_ASSETS=$(setup-envtest use -p path 1.35.0)
```
