...
func BenchmarkAuthPipeline(b *testing.B) {
	...
	idConfig := &evaluators.IdentityConfig{JWTAuthentication: identity.NewJWTAuthentication(context.TODO(), identity.NewOIDCProviderVerifier(context.TODO(), fmt.Sprintf("http://%v", oidcServerHost), 0), authCredMock)}
