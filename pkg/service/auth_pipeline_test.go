...
func BenchmarkAuthPipeline(b *testing.B) {
	...
	idConfig := &evaluators.IdentityConfig{JWTAuthentication: identity.NewJWTAuthentication(identity.NewOIDCProviderVerifier(context.TODO(), fmt.Sprintf("http://%v", oidcServerHost), 0, nil), authCredMock)}
