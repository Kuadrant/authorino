...
func (self *AuthService) Check(ctx context.Context, req *auth.CheckRequest) (*auth.CheckResponse, error) {
	...
	if err != nil { return self.deniedResponse(rpc.FAILED_PRECONDITION, "Invalid request"), nil }
	log.Println("[AuthService] Check()\n", string(reqJSON))

	// service config
	host := req.Attributes.Request.Http.Host
	apiConfig, apiConfigOK := self.Config.APIs[host]
  if !apiConfigOK || !bool(apiConfig.Enabled) {
	...
	if err != nil { return self.deniedResponse(rpc.PERMISSION_DENIED, err.Error()), nil }
