...
// pemToRFC9440 converts a PEM-encoded certificate to RFC 9440 format
// RFC 9440 format: :base64_encoded_DER_cert:
func pemToRFC9440(pemCert []byte) string {
...
func TestCallWithClientCertHeader(t *testing.T) {
	var data []byte
	...
	selector, _ := k8s_labels.Parse("app=all")
	// Create MTLS evaluator configured to extract from Client-Cert header (RFC 9440)
	mtls := NewMTLSIdentity("mtls", selector, "ns1", "", "client-cert", nil, testMTLSK8sClient, context.TODO())
	pipeline := mock_auth.NewMockAuthPipeline(ctrl)

	// john (ca: pets) - certificate in Client-Cert header (RFC 9440 format)
	// RFC 9440: base64-encoded DER with colon delimiters
	clientCertValue := pemToRFC9440(testCerts["john"]["tls.crt"])

	pipeline.EXPECT().GetHttp().Return(&envoy_auth.AttributeContext_HttpRequest{
		Headers: map[string]string{
			"client-cert": clientCertValue,
		},
	})
	obj, err := mtls.Call(pipeline, context.TODO())
	assert.NilError(t, err)
	...
	// aisha (ca: cars) - certificate in Client-Cert header
	clientCertValue = pemToRFC9440(testCerts["aisha"]["tls.crt"])
...
func TestCallWithClientCertHeaderMissing(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	...
	// Client-Cert header not present
	pipeline.EXPECT().GetHttp().Return(&envoy_auth.AttributeContext_HttpRequest{
		Headers: map[string]string{},
	})
	obj, err := mtls.Call(pipeline, context.TODO())
	assert.Check(t, obj == nil)
	assert.ErrorContains(t, err, "header client-cert not found")
}

func TestCallWithClientCertHeaderInvalidCert(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	...
	// Invalid certificate in Client-Cert header (missing colon delimiters)
	pipeline.EXPECT().GetHttp().Return(&envoy_auth.AttributeContext_HttpRequest{
		Headers: map[string]string{
	...
	obj, err := mtls.Call(pipeline, context.TODO())
	assert.Check(t, obj == nil)
	assert.ErrorContains(t, err, "missing colon delimiters")
}

func TestCallWithClientCertHeaderUnknownAuthority(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	...
	// niko (ca: books) - not trusted in ns1
	clientCertValue := pemToRFC9440(testCerts["niko"]["tls.crt"])
...
func TestCallWithClientCertHeaderCaseInsensitive(t *testing.T) {
	var data []byte
	...
	// Headers are normalized to lowercase in Envoy
	pipeline.EXPECT().GetHttp().Return(&envoy_auth.AttributeContext_HttpRequest{
		Headers: map[string]string{
			"client-cert": clientCertValue,
		},
