...
func startExtAuthServerGRPC(authConfigIndex index.Index, opts authServerOptions) {
	...
	envoy_auth.RegisterAuthorizationServer(grpcServer, &service.AuthService{Index: authConfigIndex, Timeout: timeoutMs(opts.timeout), EnableLoggingFields: opts.enableLoggingFields})
...
func startExtAuthServerHTTP(authConfigIndex index.Index, opts authServerOptions) {
	startHTTPService("auth", opts.extAuthHTTPPort, service.HTTPAuthorizationBasePath, opts.tlsCertPath, opts.tlsCertKeyPath, opts.tlsMinVersion, opts.tlsMaxVersion, opts.tlsCipherSuites, service.NewAuthService(authConfigIndex, timeoutMs(opts.timeout), opts.maxHttpRequestBodySize, opts.enableLoggingFields))
}
